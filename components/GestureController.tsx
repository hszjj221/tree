...
import { MEDIAPIPE_CONFIG, GESTURES } from '../types';
...
const GestureController: React.FC<GestureControllerProps> = ({ isSnowEnabled, setSnowEnabled }) => {
  ...
  const [currentGesture, setCurrentGesture] = useState<string>(GESTURES.NONE);
  ...
  const predictWebcam = () => {
    ...
    if (video.currentTime !== lastVideoTimeRef.current) {
      ...
      if (canvasRef.current) {
        ...
        if (ctx) {
          ...
          if (results.landmarks && results.landmarks.length > 0) {
            ...
            const gesture = results.gestures.length > 0 ? results.gestures[0][0].categoryName : GESTURES.NONE;

            let color = 'rgba(255, 255, 255, 0.8)';
            if (gesture === GESTURES.OPEN_PALM) color = '#4ade80'; // Green
            if (gesture === GESTURES.CLOSED_FIST) color = '#f87171'; // Red

            drawSkeleton(landmarks as NormalizedLandmark[], ctx, width, height, color);
          } else {
            ctx.clearRect(0, 0, width, height);
          }
        }
      ...
      if (results.gestures.length > 0) {
        ...
        // Use configured threshold (lowered from 0.5 to 0.35 for better sensitivity)
        if (score > MEDIAPIPE_CONFIG.gestureThreshold) {
          setCurrentGesture(category);

          if (category === GESTURES.OPEN_PALM) {
            setSnowEnabled(true);
          } else if (category === GESTURES.CLOSED_FIST) {
          ...
          <div className="absolute bottom-0 left-0 right-0 h-1 bg-white/20">
            <div
              className={`h-full transition-all duration-300 ${
                currentGesture === GESTURES.OPEN_PALM ? 'bg-green-500 w-full' :
                currentGesture === GESTURES.CLOSED_FIST ? 'bg-red-500 w-full' : 'w-0'
              }`}
            />
          </div>
        </div>
